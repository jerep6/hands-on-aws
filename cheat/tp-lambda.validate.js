...
exports.handler = function(event, context) {
  ...
  function validate(arrayOfLine) {
    ...
    const validPoi =  _.filter(arrayOfLine, elt => {
      let isValid = true;
      if(_.get(elt, 'datasheet.address.countryCode') !== 'FRA') {
      ...
      if(_.get(elt, 'datasheet.hotel_stars') < 0  || _.get(elt, 'datasheet.hotel_stars') > 5) {
      ...
      if(_.get(elt, 'datasheet.pricemin_gm21') <= 0) {
    ...
    logger.debug(`Number of valid POI ${validPoi.length}`, _.assign({count: validPoi.length, ids: _.map(validPoi, "uuid"), etape: "validate", durationFct: timerFct.elapseTime()}, logMetadata));
    return validPoi;
  }
